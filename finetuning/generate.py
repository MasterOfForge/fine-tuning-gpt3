...
def create_scenario():
    for scenario in scenarios:
        prompt = open_file('.vscode/pjfinetuning/prompt_bulletpoints.txt').replace('<<SCENARIO>>', scenario).replace('<<UUID>>', str(uuid4()))
        print('\n\n----------\n\n', prompt)
...
def final_prompt():
    ...
    for file in os.listdir(src_dir):
        ...
        prompt = open_file('.vscode/pjfinetuning/email_bulletpoint.txt').replace('<<SCENARIO>>', scenario)
        print('\n\n==========\n\n', prompt)
        completion = gpt_completion(prompt)
        print('\n\n', completion)
